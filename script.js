...
const SMALL_SCREEN=matchMedia('(max-width:760px)').matches;
...
/* Touch devices hide the custom cursor in CSS — don't run its rAF loop or listeners there. */
if(POINTER_FINE&&dot&&ring){
...
}else{dot&&dot.remove();ring&&ring.remove()}
...
const glitter={particleCount:SMALL_SCREEN?80:289,colors:['#90bef5','#1176f0','#90bef5'],speed:2,density:74,starSize:16,focalDepth:6,brightness:.61,intensity:2};
...
function resizeGlitter(){const dpr=Math.min(devicePixelRatio||1,SMALL_SCREEN?1:1.5),w=innerWidth,h=innerHeight;if(w===glitterSize.w&&h===glitterSize.h&&dpr===glitterSize.dpr)return;glitterSize={w,h,dpr};canvas.width=w*dpr;canvas.height=h*dpr;canvas.style.width=w+'px';canvas.style.height=h+'px';ctx.setTransform(dpr,0,0,dpr,0,0);ctx.clearRect(0,0,w,h)}
...
/* Spline is a live WebGL scene behind a CSS filter and blend mode — the single most
   expensive thing on the page. Phones never load it at all; desktop loads the viewer
   lazily and stops painting the scene once the hero scrolls away. */
const heroSection=document.querySelector('.hero'),heroSpline=document.querySelector('.hero-spline');
if(heroSpline&&SMALL_SCREEN){heroSpline.remove()}
else if(heroSpline){
